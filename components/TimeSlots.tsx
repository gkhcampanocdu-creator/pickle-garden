      ...
      <div className="grid grid-cols-[repeat(auto-fill,minmax(88px,1fr))] gap-1.5">
        {Array.from({ length: hEnd - hStart }, (_, i) => {
          ...
          if (isPast) {
            ...
            wrapClass = 'bg-stone-50 border border-stone-100 opacity-35 cursor-not-allowed'
            timeClass = 'text-stone-300'
            tagText   = 'Too late'
            tagClass  = 'text-stone-300'
          } else if (isConflict) {
            ...
            wrapClass = 'bg-white border border-stone-200 hover:border-[#006241] hover:bg-[#F0F7F4] cursor-pointer'
          ...
          return (
            <div
              ...
              aria-label={clickable ? `Select ${h12(h)}` : undefined}
              onClick={() => isAvailable && onPickHour(h)}
              onKeyDown={e => {
                if ((e.key === 'Enter' || e.key === ' ') && isAvailable) onPickHour(h)
              }}
              className={`rounded-lg py-2.5 px-2 text-center transition-all duration-150 select-none ${wrapClass}`}
            >
